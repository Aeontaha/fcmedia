		...
		init: function () {
			(function ($) {
				...
				document.addEventListener("DOMContentLoaded", function () {
					...
					const callback = function (mutationsList, observer) {
						for (let mutation of mutationsList) {
							if (mutation.type === "childList") {
								const currentText = fancyText.innerText;
								if (currentText.includes("Découvrez l’efficacité de notre Data Enriched Approach")) {
									counter.style.display = "none";
									fancyText.classList.add("mm-top");
								} else if (currentText.includes("Points de contact à travers tout le Maroc")) {
									counter.style.display = "block";
									fancyText.classList.remove("mm-top");
								}
							}
						}
					};
					...
					observer.observe(fancyText, config);
				});
				...
				function fullScreenHeight() {
					...
					if (!$("header").hasClass("sticky")) {
						fullScreenObj.parents("section").imagesLoaded(function () {
							if (
								($("section:first.full-screen, section:first .full-screen").length && ($(".top-space-margin").length || $(".mobile-top-space").length)) ||
